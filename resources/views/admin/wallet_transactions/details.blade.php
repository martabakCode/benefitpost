    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <h3 class="text-teal-950 text-xl font-bold mb-5">
                ...
                <div class="flex flex-row gap-x-5 items-center justify-between">
                    <svg width="100" height="100" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div>
                            ...
                            <h3 class="text-teal-950 text-xl font-bold">Rp {{ number_format($walletTransaction->amount, 0, ',','.' )}}</h3>
                        </div>
                        ...
                        <div>
                            ...
                            <h3 class="text-teal-950 text-xl font-bold">{{ $walletTransaction->created_at->format('M d, Y') }}</h3>
                        </div>
                        <div class="">
                            <p class="text-slate-500 text-sm">User</p>
                            <h3 class="text-teal-950 text-xl font-bold">{{ $walletTransaction->user->name }}</h3>
                        </div>
                </div>
                ...
                <hr class="my-5">
                    ...
                    <div>
                        <h3 class="text-teal-950 text-xl font-bold mb-5">Send Payment to:</h3>
                <div class="flex flex-row gap-x-10">
                    <div>
                        <p class="text-slate-500 text-sm">Bank</p>
                        <h3 class="text-teal-950 text-xl font-bold">{{ $walletTransaction->bank_name }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">No Account</p>
                        <h3 class="text-teal-950 text-xl font-bold">{{ $walletTransaction->bank_account_number }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Account Name</p>
                        <h3 class="text-teal-950 text-xl font-bold">{{ $walletTransaction->bank_account_name }}</h3>
                    </div>
                </div>
                        ...
                        @if($walletTransaction->is_paid)
                        <hr class="my-5">
                        <h3 class="text-teal-950 text-xl font-bold mb-5">Proof of Payment</h3>
                    ...
                    @if(!$walletTransaction->is_paid)
                    <h3 class="text-teal-950 text-xl font-bold">Confirm Withdrawal</h3>
                    <form method="POST" action="{{ route('admin.wallet_transactions.update', $walletTransaction) }}" enctype="multipart/form-data">
                        ...
                        <div class="flex items-center justify-end mt-4">
                            ...
                            <button type="submit" class="font-bold py-4 px-6 bg-teal-700 text-white rounded-full">
                                Update Withdrawal
                            </button>
                        </div>
                    </form>
                ...
                <hr class="my-5">
                <h3 class="text-teal-950 text-xl font-bold mb-5">Proof of Topup Payment</h3>
                        <img src="{{ Storage::url($walletTransaction->proof) }}" alt="" class="rounded-2xl object-cover w-[300px] h-[200px] mb-3">
