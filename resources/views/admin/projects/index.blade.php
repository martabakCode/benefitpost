<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row justify-between items-center">
            ...
            <a href="{{ route('admin.projects.create') }}" class="font-bold py-4 px-6 bg-teal-700 text-white rounded-full">
                Add New
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-col md:flex-row gap-y-10 justify-between md:items-center">
                    ...
                    <div class="hidden md:flex flex-col">
                        ...
                        <h3 class="text-teal-950 text-xl font-bold">Rp {{ number_format($project->budget, 0, ',','.' )}}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Applicants</p>
                        <h3 class="text-teal-950 text-xl font-bold">{{ $project->applicants->count() }}</h3>
                    </div>
                    <div class="hidden md:flex flex-col">
                        <p class="text-slate-500 text-sm">Client</p>
                        <h3 class="text-teal-950 text-xl font-bold">{{ $project->owner->name }}</h3>
                    </div>
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <a href="{{ route('admin.projects.show', $project) }}" class="font-bold py-4 px-6 bg-teal-700 text-white rounded-full">
                            Manage
                        </a>
                    </div>
                </div>
